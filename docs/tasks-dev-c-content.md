...
- **서브에이전트 1개**가 그 100개를 전부 맡음 (terms.json 항목 + aliases + related + terms/<slug>.html 페이지 생성까지 한 번에). 5개로 쪼개서 병렬 실행하지 말 것.
- 100개가 서브에이전트 컨텍스트에 부담되면 최대 2개까지만 분할 (그 이상 쪼개지 말 것)
- 새 용어의 `related`는 대충 채우고 넘어가지 말 것 — 각 신규 용어마다 실제로 연관 있는 기존/신규 slug 2~5개를 골라 채운다 (같은 category라고 기계적으로 묶지 말고, 실제로 링크를 눌렀을 때 말이 되는 것만).
- **배치 완료 직후 반드시 관련 용어 자동 보강 + 검증을 실행**:
  ```
  npm run build:related
  ```
  이 명령이 `import:related`(anova.html류 기존 HTML의 관련용어 링크를 terms.json에 반영) →
  `suggest:related`(비어있는 related 자동 추천 채움, 기존 관계는 보존) →
  `check:related`(양방향 일관성 검증) → `build:related-html`(HTML의 관련용어 블록 재생성) →
  `check:related`(재검증) → `build:terms-index`까지 한 번에 처리한다. **`check:related` 에러가
  0건이 아니면 이번 배치가 원인인지 확인하고 고친 뒤 다음 100개로 넘어갈 것** — 기존에 있던
  다른 배치발 오류까지 이번에 다 고칠 필요는 없지만, 최소한 이번에 추가한 용어 때문에 새 에러가
  생기면 안 됨.
- 한 배치(100개) 완료 후 결과 중 10~15개 스팟체크(slug 중복, JSON 유효성, aliases 확신도, related가
  실제로 연관 있는지) 하고 다음 100개로 넘어감
- **push 시점**: 문서/루틴 수정처럼 콘텐츠 변화가 없는 커밋은 즉시 push하지 말 것 — 실제 용어 배치 추가처럼 의미 있는 변경과 묶어서, 혹은 명시적으로 요청받았을 때만 push
...
## 2. 관련 용어 보완 (→ 1번 신규 용어 추가 루틴에 흡수됨)

**현재 문제**: [terms/anova.html](../terms/anova.html)의 "관련 용어" 섹션처럼, 대부분 페이지에 관련 용어 링크가 수작업으로 몇 개씩만 들어가 있거나 아예 없음.

**상태 (2026-07-26)**: 개발자 A의 `suggest-related.js`/`check-related-consistency.js`가 준비됐고,
이 검수 단계는 1번 항목의 "대량 신규 용어 추가 시" 루틴에 `npm run build:related` 실행 +
`check:related` 통과를 필수 스텝으로 넣는 방식으로 흡수했습니다. **앞으로 신규 용어를 추가할
때마다 자동으로 이 검수가 따라가므로, 이 항목을 별도로 다시 돌 필요는 없습니다.**

**남은 부채**: 이번 루틴 반영 이전에 추가된 기존 용어들 사이에는 `npm run check:related` 기준
약 1500건 이상의 양방향 불일치 오류가 남아있습니다(`related`가 A→B로만 걸려있고 B→A가 없는
경우가 대부분). 이건 과거분 정리 작업이라 이번 루틴 변경과는 별개로 시간 날 때 처리:
```
npm run build:related
```
한 번 돌리면 `suggest-related.js`의 양방향 보정 로직이 상당수를 자동으로 고쳐줄 가능성이 높음 —
돌린 뒤 `check:related` 에러 수 변화와 diff를 확인하고, 그래도 남는 진짜 이상한 조합만 수작업으로
정리하면 됨.

**완료 기준 (과거분 정리 시)**
- `npm run check:related` 실행 결과 에러 0건
- 자동 보정으로 새로 생긴 관련어 링크가 실제로 말이 되는지(엉뚱한 카테고리 조합 없는지) 샘플 검토
